...
<head>
    ...
    <link rel="stylesheet" href="{{ url('stile_profile.css') }}">
    ...
    <script src="{{ url('change_password.js') }}" defer="true"></script>
    <script src= "{{ url('insert_object.js') }}" defer="true"></script>
</head>
<body>
    <div id="navbar">
        <h1> FANTACALCIO </h1>
    ...
    <div id="search">
        <header>
            <nav>
                <div id="logo">
                    <img src="{{ url('immagini/logo-fantacalcio.svg') }}">
                </div>
                <div id="Menu">
                    <a href="{{ url('home') }}"> HOME </a>
                    <a href="{{ url('fanpage') }}"> FANPAGE </a>
        ...
        <section id="info-utente">
            ...
            <div class="dati">
                <div class="item">
                    ...
                    <input type="text" id="name" value="{{ $utente->nome }}" disabled>
                </div>
                <div class="item">
                    <label for="surname">COGNOME:</label>
                    <input type="text" id="surname" value="{{ $utente->cognome }}" disabled>
                </div>
                <div class="item">
                    <label for="username">USERNAME:</label>
                    <input type="text" id="username" value="{{ $utente->username }}" disabled>
                </div>
                <div class="item">
                    <label for="email">INDIRIZZO EMAIL:</label>
                    <input type="text" id="email" value="{{ $utente->email }}" disabled>
                </div>
            </div>
            <h1 class="titolo">Password di Accesso</h1>
            <form action="{{ url('ChangePassword') }}" id="modifica-password" method="POST">
                @csrf
                <div class="dati">
                    <div class="options">
